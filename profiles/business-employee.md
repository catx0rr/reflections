...
- **Host scheduling** (when the cron fires the prompt): top-level `dispatchCadence`. That is the only scheduling control.
- **Mode-due check** (which modes the prompt runs): the runtime prompt's Step 0 applies **hardcoded elapsed-interval** gating — rem=6h, deep=12h, core=24h — compared against each mode's `lastRun` timestamp. Per-mode `modes.*.cadence` fields are not consumed by the runtime and are not part of the preset.
...
- **Merges** duplicate candidates (`duplicate_of_existing` resolves in the index, no hard-trigger) into the existing durable node instead of creating a duplicate. Reinforces in place — bumps `referenceCount`, `lastReferenced`, and appends to `mergeKeys` / `reinforcedBy[]`.
- **Compresses** repeated weak ops material with a stable `trendKey` (observation/status/trend memory types without actionable procedure) into a single trend node at `TRENDS.md`. Upserts the trend entry (`memoryType: "trend"`) and bumps `trendSupportCount` / `trendLastUpdated`.
